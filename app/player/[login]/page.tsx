...
import { Trophy, Swords, TrendingUp, Calendar, ExternalLink, ArrowLeft } from 'lucide-react';
  ...
  const loadPlayerData = async () => {
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        if (isWinner || isLoser) {
          battles.push({
            ...
            opponentFinalHp: isWinner ? data.loser?.final_hp : data.winner?.final_hp,
            turns: data.stats?.total_turns || 0,
            hadTypeAdvantage: isWinner ? data.stats?.winner_had_type_advantage : false,
          });
      ...
      setStats({
        ...
        averageTurns: battles.length > 0 ? Math.round(totalTurns / battles.length) : 0,
    ...
    return (
      <div className="flex items-center justify-center py-12 min-h-screen h-full">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">Loading player profile...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 text-center">
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400">Total Battles</div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 text-center">
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400">Total Damage</div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 text-center">
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400">Avg. Turns</div>
          </div>
        </div>

        {/* Fighter Stats */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 mb-6">
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4">Fighter Stats</h2>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-red-600 dark:text-red-400 font-bold mb-1">ATK</div>
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{fighter.card.attack}</div>
            </div>
            <div>
              <div className="text-blue-600 dark:text-blue-400 font-bold mb-1">DEF</div>
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{fighter.card.defense}</div>
            </div>
            <div>
              <div className="text-yellow-600 dark:text-yellow-400 font-bold mb-1">SPD</div>
              <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{fighter.card.speed}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6">
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4">Recent Battles</h2>
          {stats.recentBattles.length === 0 ? (
            <p className="text-center text-slate-600 dark:text-slate-400 py-8">No battles yet</p>
          ) : (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-slate-600 dark:text-slate-400">
                        {battle.turns} turns • HP: {battle.finalHp} vs {battle.opponentFinalHp}
                        {battle.hadTypeAdvantage && ' • Type Advantage'}
                      </div>
                    </div>
                  </div>
                  <div className="text-sm text-slate-500 dark:text-slate-500">
                    {battle.timestamp?.toDate?.().toLocaleDateString() || 'Unknown'}
                  </div>
                </div>
              ))}
            </div>
